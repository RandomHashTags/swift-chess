...
public struct ChessPosition: CustomStringConvertible, Hashable, Sendable {
    ...
    public init?(algebraicNotation: some StringProtocol) {
        guard algebraicNotation.count == 2,
            let file = algebraicNotation[algebraicNotation.startIndex].lowercased().first?.asciiValue,
            let rank = algebraicNotation[algebraicNotation.index(after: algebraicNotation.startIndex)].lowercased().first?.asciiValue
        else {
            return nil
        }
